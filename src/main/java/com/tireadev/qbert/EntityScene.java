...
public class EntityScene extends Scene{
    ...
    public void onUpdate(float v) {
        ...
        if (keyPressed('W') && qPos.y > 0 && qPos.x >= 0){
            dir.x = -1;
            dir.y = -1;
            move(qPos, dir);
        }else if (keyPressed('S') && qPos.y < 6 && qPos.x < MapScene.mapWidth){
            dir.x = 1;
            dir.y = 1;
            move(qPos, dir);
        }else if (keyPressed('A') && qPos.y < 6 && qPos.x >= 0){
            dir.x = -1;
            dir.y = 1;
            move(qPos, dir);
        }else if (keyPressed('D') && qPos.y > 0 && qPos.x < MapScene.mapWidth){
            dir.x = 1;
            dir.y = -1;
            move(qPos, dir);
        }
